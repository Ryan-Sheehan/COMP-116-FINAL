...
const Thanks = () => {
  return(
    React.createElement("div", {className: "thanks-container"},
    React.createElement("img", { className: "thanks-icon", src: "thanks.png" }))
...
React.createElement("div", { className: "shop-item-container" },
  React.createElement("img", {className: "item-sold-out-sticker", src: 'sold-out.png'})),
...
const AddItem = ({ id, count, onSubmit }) => {
  ...
  if (count <= 0) {
    return ( 
      React.createElement("img", {className: "item-sold-out", src: 'sold-out.png'}))
...
{
  id: ownProps.id,
  count:  initialState.stock.find(item => item.id === ownProps.id).count,
...
React.createElement("form", {className: "form", action: "/"},
React.createElement("div", {className: "form-container"},

React.createElement("div", {className: "name"},
React.createElement("div", {className: "name-label"}, "Name"),
React.createElement("input", {id: "name", type:"text", required: "required"}),
React.createElement("img", {className: "form-sticker", src: "ny-times.png"})),

React.createElement("div", {className: "phone-number"}, 
React.createElement("div", {className: "phone-label"}, "Phone Number"),
React.createElement("input", {id: "phone", type:"tel", required: "required"})),

React.createElement("div", {className: "address"},
React.createElement("div", {className: "address-label"}, "Delivery Address"),
React.createElement("input", {id: "address", type:"text", required: "required"})),

React.createElement("div", {className: "time"},
React.createElement("div", {className: "time-label"}, "When to deliver (optional)"),
React.createElement("input", {id: "time", type:"text"})),

React.createElement("div", {className: "instructions"},
React.createElement("div", {className: "instructions-label"}, "Special Instructions (optional)"),
React.createElement("input", {id: "instructions",type:"text"})),

React.createElement("div", {className: "venmo"},
React.createElement("div", {className: "venmo-check-label"}, "Did you Venmo @yum-t86? \n (we'll know 👀)")),

React.createElement("input", {type:"checkbox", required: "required"})), 
React.createElement(Link, { to: "/thanks" }, 
...
Object.assign({}, ownProps, stateProps, dispatchProps, {
  onPayClick: () => {
    ...
    console.log("HELLO RYAN: " + name)
    console.log("HELLO RYAN: " + phone)
    console.log("HELLO RYAN: " + address)
    console.log("HELLO RYAN: " + time)
    console.log("HELLO RYAN: " + instructions)
    
  stateProps.cart.map(item => {
    console.log("HELLO RYAN: " + item.count)
    dispatchProps.dispatch(removeStockItem(item.id, item.count));
    dispatchProps.dispatch(removeFromCart(item.id));
  }) 

  console.log(stateProps.cart)

  axios.get('https://stark-crag-55647.herokuapp.com/get-stock')
  .then(response => {
    console.log(response.data.stock);
    initialState = response.data;
    console.log("serverState: " + JSON.stringify(initialState));
    
    var customerCart = stateProps.cart
    var customerInfo = {name: name, phone: phone, address: address, time: time, instructions: instructions};
  
    var newOrder = {customerCart, customerInfo}
    
      axios.post(`https://stark-crag-55647.herokuapp.com/send-email/`, {customerCart, customerInfo})
      .then(res => {
...
React.createElement(Route, { path: "thanks", component: ThanksContainer }),
